...
//COLLISION
__kernel void alpha(const param_t params, __global speed_t* cells, __global char* obstacles, __global float* tot_u, const accel_area_t accel_area)
{
  ...
  if (obstacles[ii*params.nx + jj])
  {
      ...
      ** mirroring, and writing into main grid */
      tmp[1] = cells[ii*params.nx + jj].speeds[3];
      tmp[2] = cells[ii*params.nx + jj].speeds[4];
      tmp[3] = cells[ii*params.nx + jj].speeds[1];
      tmp[4] = cells[ii*params.nx + jj].speeds[2];
      tmp[5] = cells[ii*params.nx + jj].speeds[7];
      tmp[6] = cells[ii*params.nx + jj].speeds[8];
      tmp[7] = cells[ii*params.nx + jj].speeds[5];
      tmp[8] = cells[ii*params.nx + jj].speeds[6];
    ...
    {
        tmp[kk] =
            (cells[ii*params.nx + jj].speeds[kk] + params.omega *
            (d_equ[kk] - cells[ii*params.nx + jj].speeds[kk]));
    }
...
__kernel void beta(const param_t params, __global speed_t* cells, __global char* obstacles, __global float* tot_u, const accel_area_t accel_area)
{
  int ii,jj,kk;  /* generic counters */
  ii = get_global_id(1);
  jj = get_global_id(0);
